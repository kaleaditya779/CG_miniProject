...
//  FUNCTIONS USED
  ...
  void main()
 {
   ...
   void starting()
   {
	...
	setcolor(WHITE);
	settextstyle(3,0,2);
	outtextxy(100,50, "Welcome to our CG miniProject..");
	outtextxy(100,80, "BY = 1) Aditya Kale");
	outtextxy(105,100, "2) Nishtha Jethani");
	outtextxy(105,120, "3) Nishigandha Deore");
	outtextxy(100,150, "Our project is about photon, atom & related");
	outtextxy(100,170, "concepts in physics");
	delay(5000);
	cleardevice();
   }
  ...
  {
    setbkcolor(BLUE); // To set background color
    setcolor(WHITE);
    circle(170,230,8);   // First object

    // Different color filling & other uses..
    ...
    delay(3000);  // Given a delay time, so that there will be sufficient
		 // time in between execution of 2 functions...

    // Then after that the screen gets cleared...
    cleardevice();
    }
